...
if ( $available_methods ) {
	...
	// Show radio buttons for methods
		...
		foreach ( $available_methods as $method )
			echo '<li><label class="checkbox" for="shipping_method_' . sanitize_title( $method->id ) . '"><input type="radio" name="shipping_method" id="shipping_method_' . sanitize_title( $method->id ) . '" value="' . esc_attr( $method->id ) . '" ' . checked( $method->id, $woocommerce->session->chosen_shipping_method, false) . ' />' . wp_kses_post( $method->full_label ) . '</label></li>';
