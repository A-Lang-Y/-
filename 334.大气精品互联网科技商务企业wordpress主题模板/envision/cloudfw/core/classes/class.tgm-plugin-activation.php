...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		/**
		 * Flag to disable the dismiss link for the notices
		 *
		 * @since 2.3.6
		 *
		 * @var boolean
		 */
		public $disable_dismiss = false;
		...
		public function notices() {
					...
					if ( isset( $plugin['version'] ) ) {
						if ( isset( $installed_plugins[$plugin['file_path']]['Version'] ) ) {
							...
							if ( version_compare( $installed_plugins[$plugin['file_path']]['Version'], $plugin['version'], '<' ) ) {
								...
								if ( isset($plugin['required_version']) && $plugin['required_version'] ) {
									$this->disable_dismiss = true;
								}
							}
						}
			...
			if ( ! get_user_meta( get_current_user_id(), 'tgmpa_dismissed_notice', true ) || $this->disable_dismiss ) {
				...
				if ( ! empty( $message ) ) {
					...
					foreach ( $message as $type => $plugin_groups ) {
						...
						foreach ( $plugin_groups as $plugin_group_single_name ) {
							...
							elseif ( ! $source || preg_match( '|^http://wordpress.org/extend/plugins/|', $source ) ) {
								...
								if ( $this->_get_plugin_data_from_name( $plugin_group_single_name, 'required_version' ) && $this->_get_plugin_data_from_name( $plugin_group_single_name, 'min_version_message' ) ) {
									$linked_plugin_groups[] = sprintf( $this->_get_plugin_data_from_name( $plugin_group_single_name, 'min_version_message' ),
										$plugin_group_single_name,
										$this->_get_plugin_data_from_name( $plugin_group_single_name, 'version' )
									);
								} else {
					...
					if ( $this->disable_dismiss ) {
						if ( isset($action_links['dismiss']) ) {
							unset($action_links['dismiss']);
						}
					}
