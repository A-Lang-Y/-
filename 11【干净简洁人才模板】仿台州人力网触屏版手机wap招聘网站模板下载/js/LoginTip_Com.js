 $(function(){
	...
	var cookiesval;
	if($.cookie("com_username")!=null){
	 cookiesval=$.cookie("com_username");
	}
	else
	{
	cookiesval="";
	...
	var html = '<div class="popBox" style="visibility: visible; position: fixed;width: 250px; height: auto; z-index: 35;  opacity: 1;"><a class="box-close" title="关闭"><i></i></a><div class="box-title">企业用户登录</div><div class="box-content"><ul class="pop_login"><li class="find_pwd">企业用户登录 </li><li class="user_name"><input id="username" name="username" type="text" value="'+cookiesval+'" tabindex="1" placeholder="用户名"></li><li class="password"><input id="password" name="password" type="password" tabindex="2" placeholder="密码"></li><li class="login_free"><input id="Hidden1" type="hidden" value="1" /><input type="checkbox" value="1" id="loginFree" class="inputCheckbox" checked=""><label for="loginFree">30天内自动登录</label></li><li class="btn_login"><button type="button" class="btn_orange1 box-ok" tabindex="3">登录</button><button class="btn_green1" onclick="window.location.href=\'/touch/reg/\'" type="button" tabindex="4">注册</button></li></ul></div></div>';
	        $('.overlay').append($(html));
	        var $t = $('.popBox');
